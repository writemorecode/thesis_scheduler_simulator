...
def best_fit_dot(
    ...
    """
    Best-fit-style packing using normalized dot product to pick open bins.

    For each item instance, choose the open bin whose normalized remaining capacity
    has the minimum dot product with the normalized demand vector. When no open bin
    can fit the item, open a new bin using marginal cost selection.
    """
    ...
    for j in range(J):
        ...
        for _ in range(count):
            chosen_idx = _select_open_bin_min_dot(
                demand_vec, remaining, bin_types, opening_costs
            )
            ...
            if chosen_idx is None:
                ...
                new_remaining = (C[:, [bin_type]] - demand).copy()
                remaining = np.hstack((remaining, new_remaining))
                bin_types = np.concatenate((bin_types, np.array([bin_type], dtype=int)))

                new_B = np.zeros((J, 1), dtype=int)
                new_B[j, 0] = 1
                B = np.hstack((B, new_B))
            else:
                remaining[:, chosen_idx] -= demand_vec
                B[j, chosen_idx] += 1
